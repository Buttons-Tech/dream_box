      ...
      <section className="bg-emerald-900 rounded-[2rem] p-8 md:p-12 text-white relative overflow-hidden">
        <div className="relative z-10">
           ...
           <h1 className="text-4xl md:text-5xl font-black italic uppercase mt-2">
             Welcome, Professor <span className="text-yellow-400">{tutorData?.fullName?.split(' ')[0] || 'Tutor'}</span>
           </h1>
           <p className="mt-4 text-emerald-100 max-w-md font-medium">
             Your specialized track: <span className="font-bold underline">{tutorData?.specialization || 'General Faculty'}</span>
           </p>
        </div>
...
function SessionCard({ title, time, students }: any) {
  return (
    <div className="flex items-center justify-between p-6 bg-gray-50 rounded-2xl border-2 border-transparent hover:border-emerald-400 transition-all cursor-pointer">
      <div>
